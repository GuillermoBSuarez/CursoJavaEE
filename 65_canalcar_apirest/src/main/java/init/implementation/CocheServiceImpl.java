...
public class CocheServiceImpl implements CocheService {
	...
	private Coche cocheMatricula(String matricula) {
		return coches.stream()
					 .filter( c -> c.getMatrícula().equals(matricula) )
					 .findFirst().orElse(null);
	}
	
	@Override
	public void alta(Coche coche) {
		if (cocheMatricula(coche.getMatrícula()) == null) {
			coches.add(coche);
		}
	}
	...
	public Coche eliminar(String matricula) {
		Coche coche = cocheMatricula(matricula);
		coches.remove(coche);
		return coche;
	}
	...
	public void actualizar(Coche coche) {
		Coche encontrado = cocheMatricula(coche.getMatrícula());
		if (encontrado != null) {
			encontrado.setKms(coche.getKms());
